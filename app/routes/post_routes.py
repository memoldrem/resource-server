...
@posts_bp.route('/post/<post_id>', methods=['PUT']) # update
# @token_required
def update_post(post_id):
    ...
    updated_fields = {"content": data['content']}  # Modify this depending on which fields you want to update
    result = PostRepository.update_post(post_id=post_id, updated_fields=updated_fields)

    if result.get("success"):
        return jsonify({"message": result.get("message"), "post": result.get("post")}), 200
    return jsonify({"message": result.get("message")}), 400


@posts_bp.route('/post/<post_id>', methods=['DELETE']) # delete
# @token_required
